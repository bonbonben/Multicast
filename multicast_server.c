...
/*	if(sendto(sd, databuf, datalen, 0, (struct sockaddr*)&groupSock, sizeof(groupSock)) < 0)
	...
	*/
	...
	while(1)
	{
		...
		if(sendto(sd,msg,strlen(msg),0,(struct sockaddr *)&groupSock, sizeof(groupSock))==-1)
		{
			perror("send");
			exit(1);
		}
		...
		if(strcmp(msg,"file")==0)
		{
			...
			sendto(sd,filename3,1024,0,(struct sockaddr *)&groupSock, sizeof(groupSock));/*send file name*/
			char sdbuf[1024];
			...
			{
		    		if(sendto(sd,sdbuf,fs_block_sz,0,(struct sockaddr *)&groupSock, sizeof(groupSock))<0)
		    		{
		        		fprintf(stderr,"ERROR:Failed to send file %s.(errno = %d)\n",filename3,errno);
		        		break;
		    		}
		    		else bzero(sdbuf,1024);
			}
